...
class StatTestChiSquareTest: StatTestObject
	...
	// picked a random option.  Computed when computing the chi-square
	// value.
	_ev = nil

	// Number of values we've picked.  Incremented by addValue().
	_n = nil

	// Boolean flag, set when we run checkCritical().
	_success = nil
	...
	// Compute the chi-square value for the data we have.
	chiSquare() {
		local delta, ev, i, t, v;
		...
		// Our real expectation value is the product of the "base" value
		// (the inverse of the number of buckets) and the number of
		// test we ran.
		ev = _ev * new BigNumber(_n);

		_debug('bucket expectation value = <<toString(ev)>>');
		...
		// The chi-square value is the square root of the value we
		// summed up above.
		v = t.sqrt();

		_debug('chi square value = <<toString(v.roundToDecimal(3))>>');
		return(v);
	}
	...
	// evaluating the quality of our random numbers or whatever.  So
	// what we're really looking for is situations where some options
	// are *never* selected, or are selected twice as often as they should
	// be, or something like that.  And this is more than adequate for
	// catching that kind of thing.
	checkCritical(chi) {
		local cv, i, r;
